...
8.0.30, keeps `app.db` under `%LOCALAPPDATA%\SqliteShowcase\Data`,
...
the `SqliteShowcase.UI` WinUI application. Database migrations finish before the first window is
...
```powershell
dotnet ef migrations add <Name> --project SqliteShowcase.Data
```
