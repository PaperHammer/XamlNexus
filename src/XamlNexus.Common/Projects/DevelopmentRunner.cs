...
public sealed record DevelopmentRunPlan(string Root, string ProjectPath, bool Hybrid, bool NoBuild,
    DevelopmentCommand Build, DevelopmentCommand ResolveTarget);
...
public static class DevelopmentRunner {
    ...
    public static async Task<int> RunAsync(DevelopmentRunPlan plan, IDevelopmentRuntime runtime,
...
public sealed class DevelopmentRuntime(Action<string> log) : IDevelopmentRuntime {
    ...
    public IDevelopmentProcess Start(DevelopmentCommand command) {
        var process = new Process { StartInfo = CreateStartInfo(command, redirect: true) };
