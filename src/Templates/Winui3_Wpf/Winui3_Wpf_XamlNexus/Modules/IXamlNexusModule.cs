...
namespace Winui3_Wpf_XamlNexus.Modules;
...
internal interface IXamlNexusGrpcModule {
    void BindGrpcServices(ServiceBinderBase serviceBinder, IServiceProvider services);
}
...
internal sealed class XamlNexusModuleCatalog {
    ...
    public async Task InitializeAsync(
        ...
        ArgumentNullException.ThrowIfNull(services);
        foreach (IXamlNexusModule module in _modules)
            await module.InitializeAsync(services, cancellationToken).ConfigureAwait(false);
    }

    public void BindGrpcServices(ServiceBinderBase serviceBinder, IServiceProvider services) {
        ArgumentNullException.ThrowIfNull(serviceBinder);
        ArgumentNullException.ThrowIfNull(services);
        foreach (IXamlNexusGrpcModule module in _modules.OfType<IXamlNexusGrpcModule>())
            module.BindGrpcServices(serviceBinder, services);
    }
}
