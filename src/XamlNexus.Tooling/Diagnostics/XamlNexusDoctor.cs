...
public static partial class XamlNexusDoctor {
    ...
    private static void DiagnoseEnvironment(string rootDirectory, List<XamlNexusDoctorCheck> checks) {
        checks.Add(OperatingSystem.IsWindows()
            ? DoctorChecks.Windows.Create([Environment.OSVersion.Version])
            : DoctorChecks.RequiresWindows.Create());
        ...
        try {
            ...
            if (!sdkResult.Success) {
                checks.Add(DoctorChecks.SdkResolveFailed.Create([sdkResult.DiagnosticOutput]));
            }
            else {
                ...
                checks.Add(Version.TryParse(selected.Split('-')[0], out Version? version) && version.Major >= 8
                    ? DoctorChecks.SdkSelected.Create([selected])
                    : DoctorChecks.SdkUnsupported.Create([selected]));
            }
        }
        catch (Exception exception) when (
            exception is System.ComponentModel.Win32Exception or InvalidOperationException) {
            checks.Add(DoctorChecks.DotnetFailed.Create([exception.Message]));
        }
        ...
        try {
            ...
            if (!nugetResult.Success) {
                checks.Add(DoctorChecks.NugetFailed.Create([nugetResult.DiagnosticOutput]));
            }
            else {
                ...
                checks.Add(enabledSources > 0
                    ? DoctorChecks.NugetSources.Create([enabledSources])
                    : DoctorChecks.NugetMissing.Create());
            }
        }
        catch (Exception exception) when (
            exception is System.ComponentModel.Win32Exception or InvalidOperationException) {
            checks.Add(DoctorChecks.NugetFailed.Create([exception.Message]));
        }
    }
    ...
    private static void DiagnoseProject(XamlNexusProjectContext context, List<XamlNexusDoctorCheck> checks) {
        ...
        if (validation.Issues.Count == 0) {
            checks.Add(DoctorChecks.ProjectConsistent.Create());
            return;
        }
    ...
    private static void DiagnoseWindowsAppSdk(XamlNexusProjectContext context, List<XamlNexusDoctorCheck> checks) {
        ...
        foreach (string projectPath in EnumerateProjectFiles(context.RootDirectory)) {
            try {
                ...
                checks.Add(DoctorChecks.ProjectXmlUnreadable.Create([exception.Message], path: Path.GetRelativePath(context.RootDirectory, projectPath)));
            }
        }

        checks.Add(windowsProjects > 0
            ? DoctorChecks.WindowsProjectsFound.Create([windowsProjects])
            : DoctorChecks.WindowsProjectsMissing.Create());

        // 无可读取的版本声明时报错；一种版本通过；多种版本仅提示可能需要统一。
        // 因此集中管理版本等写法可能无法被此静态检查正确识别。
        if (versions.Count == 0) {
            checks.Add(DoctorChecks.AppSdkMissing.Create());
        }
        else if (versions.Count == 1) {
            checks.Add(DoctorChecks.AppSdkReferenced.Create([versions.Single()]));
        }
        else {
            checks.Add(DoctorChecks.AppSdkVersionsDiffer.Create([string.Join(", ", versions.Order())]));
        }
    }
    ...
    private static void DiagnoseRecipes(
        ...
        if (installedRecipes.Length == 0) {
            checks.Add(DoctorChecks.RecipesEmpty.Create());
            return;
        }
        ...
        foreach (XamlNexusManagedModule module in installedRecipes) {
            ...
            if (catalogRecipe is null) {
                checks.Add(DoctorChecks.RecipeUnavailable.Create([module.Id]));
            }
            // 这里只比较版本字符串是否相同，不判断哪个版本更新。
                ...
                checks.Add(DoctorChecks.RecipeVersionMatches.Create([module.Id, module.Version]));
            }
            else {
                checks.Add(DoctorChecks.RecipeVersionDiffers.Create([module.Id, module.Version, catalogRecipe.Descriptor.Version]));
            }
        }
    }
    ...
    private static void DiagnoseSqlite(
        ...
        checks.Add(hasReference
            ? DoctorChecks.DataReferencePresent.Create()
            : DoctorChecks.DataReferenceMissing.Create());
        ...
        checks.Add(hasWalText
            ? DoctorChecks.WalTextFound.Create()
            : DoctorChecks.WalTextMissing.Create());
        ...
        if (context.Manifest.Project.Preset == "hybrid") {
            ...
            checks.Add(HasProjectItem(grpcProject, "Protobuf", proto)
                ? DoctorChecks.ProtobufPresent.Create()
                : DoctorChecks.ProtobufMissing.Create());
        }
    }
    ...
    private static void DiagnosePublishing(
        ...
        if (!context.Manifest.Modules.Any(module =>
            ...
            checks.Add(DoctorChecks.ReleaseModuleMissing.Create());
            return;
        }
        ...
        try {
            ...
            checks.Add(missing.Length == 0
                ? DoctorChecks.ReleaseMetadataComplete.Create()
                : DoctorChecks.ReleaseMetadataMissing.Create([string.Join(", ", missing)], path: Path.GetRelativePath(context.RootDirectory, configPath)));
        }
        // 将文件读取或 JSON 语法错误记录为诊断项，交由报告统一展示。
        catch (Exception exception) when (exception is IOException or JsonException) {
            checks.Add(DoctorChecks.ReleaseMetadataInvalid.Create([exception.Message], path: Path.GetRelativePath(context.RootDirectory, configPath)));
        }
    }
